  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="mx-auto w-full max-w-2xl p-4 space-y-4">
        {screen === 'intro' && <IntroScreen onStart={() => setScreen('heritage')} />}
        ...
        {screen === 'result' && (
          <ResultScreen answers={answers} heritage={heritage} onRestart={restart} />
        )}
      </div>
    </div>
  )
    ...
    <div className="rounded-xl border border-border bg-card p-6 space-y-5">
      <div className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          3 phases · {QUESTIONS.length} questions · about 4 minutes. Your answers
          route you directly to the right level.
        </p>
      </div>
...
function HeritageScreen({ onAnswer }: { onAnswer: (heritage: boolean) => void }) {
  return (
    <div className="rounded-xl border border-border bg-card p-6 space-y-5">
      <div className="space-y-1.5">
      ...
      {answered && (
        <>
          <div className="rounded-lg bg-muted/50 px-3.5 py-3 text-sm leading-[1.9] text-muted-foreground">
            <TibetanText
              text={wasCorrect ? question.feedback.pass : question.feedback.fail}
            />
          </div>
...
function OptionRow({ letter, option, answered, picked, onClick }: OptionRowProps) {
          ...
          return (
            ...
            >
              ...
              <span
                className={cn(
                  'text-sm',
                  here ? cn('font-medium', LEVEL_TONE[s.tone].title) : 'text-foreground',
                )}
              >
                Level {s.id}
              </span>
              ...
              {here && (
                <Badge className={cn('h-auto shrink-0 py-0.5', LEVEL_TONE[s.tone].tint)}>
                  You are here
                </Badge>
              )}
            </div>
